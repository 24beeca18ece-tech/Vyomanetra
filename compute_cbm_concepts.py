#!/usr/bin/env python3
"""
Module 4 -- compute C1-C7 concepts for the full 50-target results/cbm_sample.csv,
plus the TOI-700 / V1828 Aql sanity-check pair from prototype.py's TARGETS.

Checkpointed exactly like run_benchmark.py / run_modules_3_5.py: each
target's row is appended to results/cbm_concepts_full.csv as soon as it
finishes, and targets already present are skipped on rerun.

Policy from results/module4_concept_diagnosis.md: n_epochs>=4 is a hard
floor for C4/C7 (enforced inside modules_4.py already); NaN above that
floor is left as-is (informative missingness), not imputed here.

Usage: python compute_cbm_concepts.py [--retry-failed] [--limit N]
"""
import os
import sys
import csv
...
OUT_CSV = os.path.join(RESULTS, "cbm_concepts_full.csv")
SAMPLE_CSV = os.path.join(RESULTS, "cbm_sample.csv")
